...
export interface CheckAttendanceResponseDto {
  id: number | null;             // ID địa điểm. null nếu ca remote (requiresLocationCheck=false) và nhân viên không gán vị trí
  name: string | null;
  latitude: number | null;
  longitude: number | null;
  radiusMeters: number | null;
  distance: number | null;       // khoảng cách (mét). null khi remote + không có vị trí
  isInRange: boolean;            // true nếu distance <= radius. **Luôn true** nếu ca remote (requiresLocationCheck=false)
  isAvailableShift: boolean;     // true nếu có ca hôm nay
  canCheckIn: boolean;           // validation tổng hợp (GPS nếu cần + time window + chưa check-in + chưa lock)
  canCheckOut: boolean;          // validation tổng hợp (GPS nếu cần + time window + chưa check-out + chưa lock)
  requiresLocationCheck: boolean; // false → FE bỏ qua isInRange/distance để gate UI, không xin GPS permission
...
export interface TodayShiftDto {
  ...
  requireCheckIn: boolean;        // false → FE ẩn nút check-in (ca chỉ check-out)
  requireCheckOut: boolean;       // false → FE ẩn nút check-out (ca chỉ check-in)
}
...
POST /v1/attendance/check-in  (multipart/form-data)
  ...
  longitude = 105.8542
    ...
    ├─ 400 "Chưa đến giờ chấm công" | "Đã hết giờ check-in. Vui lòng tạo phiếu giải trình"
    ├─ 400 "Ca làm việc hôm nay không yêu cầu check-in" (ca có requireCheckIn=false)
...
```json
{ "success": false, "error": { "code": "BAD_REQUEST", "message": "Bản ghi chấm công đã bị khóa. Vui lòng tạo đơn bù công." } }
```
...
**400** nếu ngoài khung giờ check-out (mặc định 60p trước → 60p sau giờ tan ca):
```json
{ "success": false, "error": { "code": "BAD_REQUEST", "message": "Chưa đến giờ check-out. Sớm nhất có thể check-out lúc HH:MM." } }
{ "success": false, "error": { "code": "BAD_REQUEST", "message": "Đã quá giờ check-out (sau HH:MM). Vui lòng tạo phiếu giải trình." } }
```

**400** nếu ca không yêu cầu check-out (`requireCheckOut: false`):
```json
{ "success": false, "error": { "code": "BAD_REQUEST", "message": "Ca làm việc hôm nay không yêu cầu check-out" } }
```
...
export function useAttendance() {
  ...
  const fetchTodayInfo = () =>
    get<TodayInfoResponseDto>('/v1/attendance/today-info');
  ...
  const fetchStatuses = () =>
    get<AttendanceStatusItem[]>('/v1/attendance/statuses');
  ...
  return {
    ...
    getPhotoUrl,
    fetchTodayInfo,
    fetchMyHistory, fetchAll, manualEdit,
    fetchStatuses,
  };
}
```
...
} else if (info.shift?.isOnline) {
  // Ca online — ẩn cả 2 nút, hiển thị "Hệ thống sẽ tự ghi công cuối ngày"
} else if (!info.todayRecord || !info.todayRecord.checkInAt) {
  // Chưa check-in → hiển thị nút "Check-in" NẾU ca có requireCheckIn=true, ẩn nếu false
  if (info.shift?.requireCheckIn) showCheckInButton();
  else showInfo('Ca hôm nay không yêu cầu check-in');
} else if (!info.todayRecord.checkOutAt) {
  // Đã check-in → hiển thị nút "Check-out" NẾU ca có requireCheckOut=true, ẩn nếu false
  if (info.shift?.requireCheckOut) showCheckOutButton();
  else showInfo('Ca hôm nay không yêu cầu check-out — đã hoàn tất');
} else {
...
| Check-in khi ca có `requireCheckIn=false` | 400 `"Ca làm việc hôm nay không yêu cầu check-in"` |
| Check-out khi ca có `requireCheckOut=false` | 400 `"Ca làm việc hôm nay không yêu cầu check-out"` |
| Check-out sau `shiftEnd + 60p` (default) | 400 `"Đã quá giờ check-out (sau HH:MM). Vui lòng tạo phiếu giải trình."` — window sau default là **60p**, không phải 120p như trước |
| Ca có `checkOutWindowEnd=30` → check-out sau `shiftEnd + 30p` | 400 window closed — dùng cửa sổ HR cài trên ca |
