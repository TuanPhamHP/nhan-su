...
| Method | Path | Ai được gọi | Ghi chú |
| --- | --- | --- | --- |
| GET | `/v1/general-requests/me` | Mọi user | HR/Admin: toàn bộ đơn công ty; role khác: đơn của bản thân |
...
```json
{
	"approvers": [{ "employeeId": 3 }, { "employeeId": 5 }]
}
```
...
> **Lưu ý `GET /me` cho HR/Admin:** Khi user có role `HR` hoặc `ADMIN`, endpoint này trả về **tất cả đơn của toàn công ty** (tương đương `GET /v1/general-requests`). Các role khác chỉ thấy đơn của bản thân.  
> Frontend dùng chung một endpoint `/me` — server tự phân biệt theo role.
...
export function useDocumentTemplates() {
	...
	const fetchTemplate = (id: number) => get<DocumentTemplateDetail>(`/v1/general-requests/templates/${id}`);
...
export function useGeneralRequests() {
	...
	const fetchPendingForMe = () => list<GeneralRequestResponse>('/v1/general-requests/pending-for-me');
	...
	const fetchRequest = (id: number) => get<GeneralRequestResponse>(`/v1/general-requests/${id}`);

	const createRequest = (dto: CreateRequestDto) => post<GeneralRequestResponse>('/v1/general-requests', dto);
	...
	const printRequest = (id: number) => post<GeneralRequestResponse>(`/v1/general-requests/${id}/print`, {});

	const cancelRequest = (id: number) => patch<GeneralRequestResponse>(`/v1/general-requests/${id}/cancel`, {});
...
interface GeneralRequestFcmPayload {
	type:
		| 'general_request.pending'
		| 'general_request.approved'
		| 'general_request.rejected'
		| 'general_request.cancelled';
