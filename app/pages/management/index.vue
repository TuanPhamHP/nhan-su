...
const onLeaveCount = computed(() => hrAnalytics.value?.statusBreakdown.find(s => s.status === 'ON_LEAVE')?.count ?? 0);
const inactiveCount = computed(() => hrAnalytics.value?.statusBreakdown.find(s => s.status === 'INACTIVE')?.count ?? 0);
	...
	<div class="space-y-6">
		...
		<div class="flex items-center justify-between">
			<div>
				...
				<p v-if="stats && 'departmentName' in stats && stats.departmentName" class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
			...
			>
				<svg :class="['w-4 h-4 text-gray-500', loading && 'animate-spin']" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
			...
			<template v-else-if="stats">
				...
				>
					<div class="flex items-start justify-between gap-3">
						...
						<div class="w-10 h-10 rounded-lg bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 flex items-center justify-center flex-shrink-0">
							<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
				...
				<NuxtLink
					...
					:class="pendingTotal > 0 ? 'hover:border-orange-300 dark:hover:border-orange-700' : 'hover:border-brand-300 dark:hover:border-brand-700'"
				>
					<div class="flex items-start justify-between gap-3">
						<div class="flex-1 min-w-0">
							<p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Chờ duyệt</p>
							<p :class="['mt-1 text-2xl font-bold', pendingTotal > 0 ? 'text-orange-600 dark:text-orange-400' : 'text-gray-900 dark:text-white']">
								{{ pendingTotal }}
							</p>
						...
						<div
							:class="['w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0', pendingTotal > 0 ? 'bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400' : 'bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400']"
						>
							<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
				...
				>
					<div class="flex items-start justify-between gap-3">
						...
						<div class="w-10 h-10 rounded-lg bg-teal-100 dark:bg-teal-900/30 text-teal-600 dark:text-teal-400 flex items-center justify-center flex-shrink-0">
							<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
				...
				<NuxtLink
					...
					class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-5 hover:shadow-md transition-all"
					:class="(hrAnalytics?.expiringContractsNext30Days ?? 0) > 0 ? 'hover:border-red-300 dark:hover:border-red-700' : 'hover:border-brand-300 dark:hover:border-brand-700'"
				>
					<div class="flex items-start justify-between gap-3">
						<div class="flex-1 min-w-0">
							<p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">HĐ hết hạn</p>
							<p
								:class="['mt-1 text-2xl font-bold', (hrAnalytics?.expiringContractsNext30Days ?? 0) > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-400 dark:text-gray-500']"
							>
						...
						<div
							:class="['w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0', (hrAnalytics?.expiringContractsNext30Days ?? 0) > 0 ? 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400' : 'bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500']"
						>
							<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
		...
		<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
			<!-- Dept chart: 50% on xl -->
			<div class="xl:col-span-2 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
				<p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-4">Nhân sự theo phòng ban</p>
				<div class="h-52">
					<template v-if="loading">
			...
			<div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
				<p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-4">Phân bổ độ tuổi</p>
				<div class="h-52">
					<template v-if="loading">
						<div class="flex items-end gap-2 h-full animate-pulse pb-2">
							<div v-for="n in 5" :key="n" class="flex-1 bg-gray-200 dark:bg-gray-700 rounded" :style="{ height: `${30 + n * 10}%` }"></div>
						</div>
					</template>
					<ClientOnly v-else-if="hrAnalytics">
						<DashboardAgeChart
			...
			<div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
				<p class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-4">Loại hợp đồng</p>
				<div class="h-52">
					<template v-if="loading">
						<div class="flex items-center justify-center h-full animate-pulse">
							<div class="w-32 h-32 rounded-full border-8 border-gray-200 dark:border-gray-700 border-l-transparent"></div>
						</div>
					</template>
					<ClientOnly v-else-if="hrAnalytics">
		...
		<div class="grid grid-cols-1 lg:grid-cols-5 gap-4">
			...
			<div class="lg:col-span-3 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
				...
				<template v-else-if="pendingItems.length">
					<div class="divide-y divide-gray-100 dark:divide-gray-800">
						...
						>
							<!-- Avatar initials -->
							<div class="w-8 h-8 rounded-full bg-brand-100 dark:bg-brand-900/30 text-brand-700 dark:text-brand-400 flex items-center justify-center text-xs font-bold flex-shrink-0">
			...
			<!-- Right 40%: New hires -->
			<div class="lg:col-span-2 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
				<div class="px-5 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
				...
				<template v-else-if="(hrAnalytics?.newHiresLast30Days ?? 0) > 0">
					<div class="p-5 flex flex-col gap-3">
						<div class="flex items-center gap-3 p-3 rounded-lg bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800/50">
							<div class="w-10 h-10 rounded-full bg-indigo-100 dark:bg-indigo-900/50 text-indigo-600 dark:text-indigo-400 flex items-center justify-center flex-shrink-0">
								<svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
									<path stroke-linecap="round" stroke-linejoin="round" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" />
								</svg>
							</div>
				...
				<div v-else-if="!loading" class="flex flex-col items-center justify-center py-12 text-gray-400">
					...
					<NuxtLink to="/management/employees/new" class="text-xs text-brand-600 dark:text-brand-400 hover:underline mt-1">
						Thêm nhân viên →
					</NuxtLink>
				</div>
			</div>
