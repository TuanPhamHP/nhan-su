...
const ICONS = {
	...
	checkCircle: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4',
...
function navigateActivity(targetUrl: string) {
		...
		<div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
			...
			<div v-else-if="recentActivity.length" class="divide-y divide-gray-100 dark:divide-gray-700">
				<button
					...
					@click="navigateActivity(item.targetUrl)"
				>
					<!-- Type badge -->
					<span :class="['inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium flex-shrink-0 mt-0.5', activityTypeCls(item.type)]">
						{{ item.typeLabel }}
					</span>
					...
					<div class="flex-1 min-w-0">
						<p class="text-sm font-medium text-gray-900 dark:text-white truncate">
							<span class="text-gray-500 dark:text-gray-400 mr-1">{{ item.employeeCode }}</span>{{ item.employeeName }}
						</p>
					...
					<div class="flex flex-col items-end gap-1 flex-shrink-0">
						<span :class="['inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium', statusBadgeCls(item.status)]">
							{{ item.statusLabel }}
						</span>
