...
async function loadUnreadAnnouncementCount() {
	try {
		const res = await fetchMyAnnouncements({ page: 1, limit: 1, isRead: false });
		unreadAnnouncementCount.value = res.meta.total;
	} catch {
		// non-critical
	}
}
...
onMounted(() => {
	if (user.value) loadUnreadAnnouncementCount();
});
...
const navSections: EmployeeNavSection[] = [
	{
		label: 'Tổng quan',
		items: [
			{
			...
			// Tạm ẩn khỏi sidebar (trang /agent-usage vẫn truy cập được bằng URL)
			// {
			// 	label: 'Mức dùng AI',
			// 	route: '/agent-usage',
