...
<script setup lang="ts">
	...
	function buildCells(year: number, month: number): DayCell[] {
		...
		for (let i = firstDay - 1; i >= 0; i--) {
			...
			result.push({
				day: prevDays - i,
				dateStr: toDateStr(py, pm, prevDays - i),
	...
	function stepMinute(delta: number) {
		...
		if (!isMinuteDisabled(m)) {
			selectMinute(m);
			nextTick(() => scrollColTo(minColRef.value, m));
		}
	}
	...
	function positionDropdown() {
		...
		style[spaceRight >= DROPDOWN_W ? 'left' : 'right'] =
			spaceRight >= DROPDOWN_W ? `${rect.left}px` : `${window.innerWidth - rect.right}px`;
