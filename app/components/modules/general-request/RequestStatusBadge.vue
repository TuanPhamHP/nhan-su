...
import type { GeneralRequestStatus } from '~/types/general-request.types';

const props = defineProps<{ status: GeneralRequestStatus }>();

const config = computed(() => {
	const map: Record<GeneralRequestStatus, { label: string; class: string }> = {
		...
		CANCELLED: { label: 'Đã thu hồi', class: 'bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-500 line-through' },
	};
