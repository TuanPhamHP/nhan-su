<script setup lang="ts">
	...
	async function submit() {
		...
		loading.value = true;
		try {
			const result = await service.bulkInit(selectedLeaveTypeId.value, selectedYear.value);
			toast.success(`Đã tạo ${result.created} balance mới, bỏ qua ${result.skipped} đã tồn tại`);
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
		<div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-md">
			...
			<div class="px-6 py-5 space-y-4">
				...
				<!-- Warning -->
				<div
					class="flex gap-2.5 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800"
				>
					<svg
					...
					<p class="text-xs text-amber-700 dark:text-amber-300">
						Sẽ tạo số dư phép cho tất cả nhân viên đang
						<strong>ACTIVE</strong>. Nhân viên đã có balance trong năm {{ selectedYear }} sẽ bị <strong>bỏ qua</strong>.
					</p>
				</div>

				<div class="flex items-center justify-end gap-3 pt-2">
					<CommonAppButton variant="outline" @click="emit('close')">Huỷ</CommonAppButton>
					<CommonAppButton :loading="loading" @click="submit">Khởi tạo</CommonAppButton>
				</div>
			</div>
		</div>
	</div>
