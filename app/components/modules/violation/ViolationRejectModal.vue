<script setup lang="ts">
	...
	const props = defineProps<{ violationRequest: ViolationRequest }>();
	...
	const onSubmit = handleSubmit(async values => {
		try {
			await service.reject(props.violationRequest.id, { reviewNote: values.reviewNote });
			toast.success('Đã từ chối phiếu vi phạm');
			emit('rejected');
		} catch (e) {
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
		<div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl w-full max-w-lg">
			...
			<div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
				<h2 class="text-base font-semibold text-gray-900 dark:text-white">Từ chối phiếu vi phạm</h2>
				<button
			...
			<div class="px-6 py-4 bg-gray-50 dark:bg-gray-800/50 space-y-2 text-sm">
				<div class="flex justify-between gap-4">
					...
					<span class="font-medium text-gray-900 dark:text-white text-right">
						{{ violationRequest.employee.fullName }}
						<span class="text-gray-400 text-xs ml-1">({{ violationRequest.employee.employeeCode }})</span>
					</span>
				</div>
				<div class="flex justify-between gap-4">
					<span class="text-gray-500 dark:text-gray-400 flex-shrink-0">Loại vi phạm</span>
					<span class="font-medium text-gray-900 dark:text-white">{{ violationRequest.typeLabel }}</span>
				</div>
				<div class="flex justify-between gap-4">
					<span class="text-gray-500 dark:text-gray-400 flex-shrink-0">Ngày vi phạm</span>
					<span class="font-medium text-gray-900 dark:text-white">{{ formatDate(violationRequest.violationDate) }}</span>
				</div>
				<div v-if="violationRequest.reason" class="flex justify-between gap-4">
					<span class="text-gray-500 dark:text-gray-400 flex-shrink-0">Lý do NV</span>
					<span class="font-medium text-gray-900 dark:text-white text-right">{{ violationRequest.reason }}</span>
				</div>
			</div>
			...
			<form class="px-6 py-4 space-y-4" @submit.prevent="onSubmit">
				...
				<div class="flex items-center justify-end gap-3 pt-2">
					...
					<CommonAppButton variant="danger" type="submit" :loading="isSubmitting">Từ chối phiếu</CommonAppButton>
				</div>
			</form>
		</div>
	</div>
